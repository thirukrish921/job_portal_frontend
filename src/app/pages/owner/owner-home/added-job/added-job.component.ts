...
@Component({
  selector: 'app-added-job',
  templateUrl: './added-job.component.html',
  styleUrls: ['./added-job.component.css']
})
export class AddedJobComponent implements OnInit {
  ...
  constructor(public jobServices: JobService) { 
    ...
    this.jobServices.readAll().subscribe((res: any) => {
      if(res.status === 404) {
        ...
        this.dataArray = this.dataArray.filter(item=>item.emailId==sessionStorage.getItem('email') );
      } else {
